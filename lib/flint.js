 ...
 * @property {boolean} initialized - Flint fully initialized
 * @property {boolean} isBotAccount - Is Flint attached to Webex using a bot account?
 * @property {boolean} isUserAccount - Is Flint attached to Webex using a user account?
 * @property {object} person - Flint person object
 * @property {string} email - Flint email
 * @property {object} webex - The Webex JSSDK instance used by flint
 *
 * @example
 * var options = {
...
function Flint(options) {
   ...
   * @property {string} webhookUrl - URL that is used for Webex API to send callbacks.
 ...
 * @function
 * @memberof Flint
 * @param {String} token - New Webex Token for Flint to use.
 ...
 *   });
 */
Flint.prototype.setWebexToken = function (token) {
 ...
 *
 * @example
 * flint.testWebexToken('Tm90aGluZyB0byBzZWUgaGVyZS4uLiBNb3ZlIGFsb25nLi4u')
 *   .then(function() {
 ...
 *   });
 */
Flint.prototype.testWebexToken = function (token) {
...
Flint.prototype.parseFile = function (message) {
   ...
   *
   * @namespace File
   * @property {string} id - Webex API Content ID
  ...
  when.reject(new Error('flint.ParseFile not yet supported'));
 ...
 */
Flint.prototype.getTrigger = function (type, triggerObject) {
...
//  * @returns {Promise.<Trigger>}
//  */
// Flint.prototype.getTrigger = function (attachmentAction) {
 ...
 */
Flint.prototype.getPerson = function (personId) {
 ...
 * @returns {Promise.<Person>}
 */
Flint.prototype.getPersonByEmail = function (personEmail) {
 ...
 * @returns {string}
 */
Flint.prototype.getPersonEmail = function (person) {
 ...
 * @returns {string}
 */
Flint.prototype.getPersonUsername = function (person) {
  if (person.emails.length >= 1) {
 ...
 * @returns {string}
 */
Flint.prototype.getPersonDomain = function (person) {
  if (person.emails.length >= 1) {
...
Flint.prototype.getFiles = function (messageId) {
  //TODO figure out how/if Webex SDK handles this
  when.reject(new Error('flint.getFiles not yet supported'));
...
/**
 * Spawns a bot in a Webex Teams Spae.
 *
 * @function
 * @memberof Flint
 * @private
 ...
 * @returns {Promise.<Boolean>}
 */
Flint.prototype.spawn = function (membership) {
    ...
    return flint.getWebhooks()
      ...
      .then(webhooks => {
        ...
        if (!flint.options.removeWebhooksOnStart) {
          ...
          if (webhooksToRemove instanceof Array && webhooksToRemove.length > 0) {
            return when.map(webhooksToRemove, webhook => flint.webex.webhooks.remove(webhook))
              .then(() => when(true))
              .catch(() => when(true));
          } else {
            return when(true);
        ...
        // else, only remove webhooks this app created
          ...
          return when.map(webhooks, webhook => flint.webex.webhooks.remove(webhook))
            .then(() => when(true))
            .catch(() => when(true));
        }
