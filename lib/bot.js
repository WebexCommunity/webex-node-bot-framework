 ...
 *
 * @function
 * @param {Object} replyMessage - Message to send to reply to.
 * @param {String|Object} message - Message to send to room. This can be a simple string, or a object for advanced use.
 * @param {String} [format=text] - Set message format. Valid options are 'text' or 'markdown'.
 * @returns {Promise.<Message>}
 *
 ...
 * 
 */
Bot.prototype.reply = function (replyMessage, message, format) {

  if (!validator.isMessage(replyMessage)) {
    return when.reject(new Error('bot.reply(): Invalid replyMessage parameter.  Must be message object.'));
  }
  // Webex insists that a reply must have a parentId that is NOT already a threaded reply
  // If that is the case for our replyMessage, find the root message as the parent.
  let parentId = (replyMessage.parentId) ? replyMessage.parentId : replyMessage.id;
