 ...
 * @param {Object} options - The options of the flint object this Bot spawns under.
 * @param {Object} webex - The webex sdk of the flint object this Bot spawns under.
...
Bot.prototype.exit = function () {
  if (!this.isGroup) {
    ...
    return this.flint.webex.memberships.remove(this.membership)
      .then(() => {
        return when(true);
      })
      .catch(() => {
...
Bot.prototype.add = function (email, asModerator) {
  ...
  var add = (e, m) => {
    if (validator.isEmail(e)) {
      return this.flint.webex.memberships.create({
        roomId: this.room.id,
        personEmail: e,
        isModerator: m
      })
        .then(membership => {
          this.flint.debug('Added "%s" to room "%s"',
...
Bot.prototype.remove = function (email) {
  ...
  var remove = e => {
    ...
    //    return this.flint.webex.memberships.list({ roomId: this.room.id, personEmail: e })
    return this.flint.webex.memberships.list({ roomId: this.room.id, personEmail: email })
      .then((memberships) => {
        let membership = memberships.items[0];
        return this.flint.webex.memberships.remove(membership);
      })
      .then(() => {
...
Bot.prototype.newRoom = function (name, emails, isTeam) {
  ...
  return this.flint.webex.rooms.create(newRoom)
    ...
    .catch(err => {
      ...
      if (newRoom && newRoom.id) {
        this.flint.webex.rooms.remove(newRoom)
 ...
 * 
 * This function will not work when flint was created
...
Bot.prototype.moderateRoom = function () {
  ...
  if (!this.isGroup || this.isTeam) {
    ...
    if (!membership.isModerator) {
      ...
      return this.flint.webex.memberships.update(membership)
        .then(() => when(this));
    } else {
      return when(this);
    }
...
Bot.prototype.implode = function () {
  ...
  return this.flint.webex.rooms.remove(this.room)
    .then(() => when(true))
    .catch(() => when(false));
};
 ...
 * flint.hears('/card please', function(bot, trigger) {
 *   bot.say({       
...
Bot.prototype.say = function (format, message) {
  ...
  if (typeof args[0] === 'object') {
    ...
    return this.flint.webex.messages.create(message);
  }
  ...
  // if message is string
    ...
    return this.flint.webex.messages.create(messageObj);
  }
...
Bot.prototype.dm = function (email, format, message) {
  ...
  if (email && validator.isEmail(email) && (typeof message === 'string' || typeof message === 'object')) {
    ...
    if (typeof message === 'string') {
      ...
      msgObj.toPersonEmail = email;
      return this.flint.webex.messages.create(msgObj);
    }
  }
...
Bot.prototype.sendCard = function (cardJson, fallbackText) {
  ...
  if (!validator.isCardJson(cardJson)) {
    return when.reject(new Error('Invalid card JSON'));
  }
  if (!fallbackText) {
  ...
  var format = this.flint.messageFormat;
...
Bot.prototype.messageStreamRoom = function (roomId, message) {
  ...
  var requestOptions = {
    ...
    headers: { 'Authorization': 'Bearer ' + this.flint.options.token },
...
Bot.prototype.roomRename = function (title) {
  if (!this.isGroup) {
    ...
    if (this.room.title != title) {
      let room = this.room;
      room.title = title;
      return this.flint.webex.rooms.update(room)
        .then((r) => {
          this.room = r;
          return when(this);
        });
...
Bot.prototype.getMessages = function (count) {
  if (this.flint.isBotAccount) {
    ...
    return this.flint.webex.messages.list({ roomId: this.room.id, max: count })
