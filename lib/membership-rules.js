  ...
  emitWithActor(event, bot, actorId) {
    try {
      if (!bot.active) {   // TODO is this sufficient or should I check in the deactive list?
        ...
        framework.debug(`Swallowing event: "${event}" associated with ` +
    ...
    return getMemberships(this.framework, bot, memberships)
      ...
      .then((invalidMember) => {
        if (invalidMember) {
          ...
          this.framework.emit('membershipRulesAction',
            'event-swallowed', 'spawn', bot, bot.id, actorId, invalidMember);
          return when(false);
        } else {
          return when(true);
        }
  ...
  shouldCallHears(lex, bot, trigger) {
    try {
      if (!bot.active) {   // TODO is this sufficient or should I check in the deactive list?
        ...
        framework.debug(`Swallowing call to framework.hears("${trigger.phrase}") ` +
