...
function Webhook(flint) {
  ...
  return function (req, res) {
    ...
    if (flint.options.webhookSecret) {
      // get webhook header to determine if security is enabled
      var sig = req.headers['x-spark-signature'] || false;
      if (!sig) {
        flint.debug('missing expected signature in webhook callback, ignoring...');
        return when(true);
      }

      //validate signature
      if (typeof flint.options.webhookSecret === 'string' && typeof sig === 'string') {
        var hmac = crypto.createHmac('sha1', flint.options.webhookSecret);
    ...
    // if (flint.options.webhookSecret && !(sig && flint.spark.webhookAuth(sig, body))) {
    //   // invalid signature, ignore processing webhook
    //   flint.debug('invalid signature in webhook callback, ignoring...');
    //   return when(true);
    // }
